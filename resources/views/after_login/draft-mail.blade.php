            ...
            <div class="box-body no-padding">
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped auto">
                  <tbody>
                  @foreach($display_mails_saved_indraft as $draft)
                  <tr>
                    <td><input type="checkbox"></td>
                    <td class="mailbox-star"><a href="#"><i class="fa fa-star text-yellow"></i></a></td>
                    <td class="mailbox-name"><a href="/display-readmail/{{$draft->id}}">{{$draft->name}}</a></td>
                    <td class="mailbox-subject"><b>{{substr($draft->subject,0,30)}}</b>
                    </td>
                    <td >-{{substr($draft->message,0,15)}}....</td>
                    <td class="mailbox-attachment">{{$draft->attachment}}</td>
                    <td class="mailbox-date">{{ date('F d, Y', strtotime($draft->created_at))}} {{ date('g:ia', strtotime($draft->created_at))}}</td>
                  </tr>
                  @endforeach
                  </tbody>
                </table>
